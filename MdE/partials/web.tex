\begin{EM}{Système Web}
    ...
    \begin{EMUCActor}{Utilisateur}
        \EMOperation{assignBal(bal, art, seuil, qte)}{Le bouton a été activé pour
            ...
            $(seuil, qte)$, respectivement le seuil et la quantité.}{Configurer les balances}
    \end{EMUCActor}
    ...
    \begin{EMUCActor}{Utilisateur}
        \EMOperation{configBal(bal, seuil, qte)}{Le bouton a été activé pour la
            balance $bal$ avec la configuration $(seuil, qte)$.}{Configurer les balances}
    \end{EMUCActor}
    ...
    \begin{EMUCActor}{Utilisateur}
        \EMOperation{scan()}{Demande d'identification de la part d'un contrôleur
            ...
            contrôleur.}{Configurer les balances}
    \end{EMUCActor}

    \EMActor{BtnVoirCommande}{Un bouton pour afficher la liste des commandes
        courante associée à l'utilisateur.}
    \begin{EMUCActor}{Utilisateur}
        \EMOperation{voirCommande()}{Le bouton à été actionné.}{Valider une commande}
    \end{EMUCActor}

    \EMActor{BtnValiderCommande}{Un bouton pour valider la liste des commandes
        courante associée à l'utilisateur.}
    \begin{EMUCActor}{Utilisateur}
        \EMOperation{validerCommande()}{Le bouton à été actionné.}{Valider une commande}
    \end{EMUCActor}

    \EMActor{InterfaceCatalogue}{Interface client pour ajouter un produit à la
        liste des commandes de l'utilisateur.}
    \begin{EMUCActor}{Utilisateur, Controleur}
        \EMOperation{listeArticle()}{La liste des produits est envoyée au client.}{MAJ d'une commande}
        \EMOperation{produitNonStock(art)}{L'article $art$ n'est plus en stock.}{MAJ d'une commande}
    \end{EMUCActor}
    ...
    \begin{EMUCActor}{Balance, Controleur}
        \EMOperation{alarm(art, qte)}{Une quantité $qte$ est demandée pour l'article
            $art$ associé à la balance afin d'être ajoutée à la commande.}{MAJ d'une commande}
    \end{EMUCActor}
    ...
    \begin{EMUCActor}{Utilisateur, Controleur}
        \EMOperation{estArticleCoché(art)}{Un article $art$ de la liste des commande
            a été coché ou décoché.}{Valider une commande}
        \EMOperation{changerQuantité(art, qte)}{Demande de modification de la
            quantité de l'article $art$ par la quantité $qte$.}{Valider une commande}
        \EMOperation{majCommande()}{Demande du $Système Web$ de mettre à jour la
            liste des commandes de l'utilisateur.}{Valider une ommande, MAJ d'une commande}
    \end{EMUCActor}
    ...
    \begin{EMUCActor}{Utilisateur, Controleur}
        \EMOperation{afficherErreur(err)}{Demande du $Système Web$ d'afficher
            l'erreur $err$ rencontrée.}{Valider une ommande, MAJ d'une commande}
        \EMOperation{produitNonDispo(art, b)}{Si $b$ est vrai, le $Système Web$
            ...
            disponible en stock. Sinon, le $Système Web$ n'affiche rien.}{MAJ d'une commande}
        \EMOperation{commandeValidée(b)}{Si $b$, le $Système Web$ demande
            ...
            le $Système Web$ n'affiche rien.}{Valider une commande}
        \EMOperation{afficherBalance(bal)}{Demande du $Système Web$ d'afficher la
            ...
            l'utilisateur.}{Configurer les balances}
        \EMOperation{afficherCommande(cmd)}{Demande du $Système Web$ d'afficher la
            liste des commandes $cmd$ de l'utilisateur.}{Valider une commande}
        \EMOperation{afficherFacture(cmd, prixTotal)}{Demande du $Système Web$
            ...
            $prixTotal$ liée à la commande.}{Valider une commande}
        \EMOperation{confirmAssignBal()}{L'assignement du produit a bien eu lieu.}{Configurer les balances}
        \EMOperation{erreurAssignBal()}{Une erreur a eu lieu pendant l'assignement
            de la balance.}{Configurer les balances}
        \EMOperation{confirmConfigBal()}{La configuration a bien eu lieu.}{Configurer les balances}
        \EMOperation{erreurConfigBal()}{Une erreur a eu lieu durant la
            configuration.}{Configurer les balances}
    \end{EMUCActor}
\end{EM}
